...
class TestTriageLabelerNeedsTriageTransitionRules(unittest.TestCase):
    """Tests for transition from needs-triage to under-review when reviewers assigned."""
    ...
    def setUp(self):
        ...
        self.mock_repo.full_name = "mock-org/mock-repo"
        self.labeler = TriageLabeler(self.mock_client, self.mock_repo, dry_run=False)
        # Mock _apply_label to verify calls
        self.labeler._apply_label = Mock()

    def test_needs_triage_pr_with_reviewer_assigned_transitions_to_under_review(self):
        """PR with status:needs-triage and reviewer assigned should transition to under-review."""
        pr = Mock(spec=github.PullRequest.PullRequest)
        pr.number = 1
        ...
        # Mock reviewer assigned
        reviewer = Mock()
        pr.requested_reviewers = [reviewer]
        pr.requested_teams = []

        self.labeler._triage_needs_triage(pr)

        self.labeler._apply_label.assert_called_once_with(pr, "status:under-review")

    def test_needs_triage_pr_with_team_assigned_transitions_to_under_review(self):
        """PR with status:needs-triage and team reviewer assigned should transition to under-review."""
        pr = Mock(spec=github.PullRequest.PullRequest)
        pr.number = 1
        ...
        # Mock team reviewer assigned
        team = Mock()
        pr.requested_reviewers = []
        pr.requested_teams = [team]

        self.labeler._triage_needs_triage(pr)

        self.labeler._apply_label.assert_called_once_with(pr, "status:under-review")

    def test_needs_triage_pr_with_no_reviewers_remains_needs_triage(self):
        """PR with status:needs-triage and no reviewers should not transition."""
        pr = Mock(spec=github.PullRequest.PullRequest)
        pr.number = 1
        ...
        # Should not call _apply_label because it already has needs-triage
        # and _is_eligible_for_triage will return False (because it has the label)
        # and has_needs_triage and has_reviewers is False.
        self.labeler._apply_label.assert_not_called()

    def test_eligible_pr_with_reviewers_goes_directly_to_under_review(self):
        """PR eligible for triage but with reviewers assigned should get under-review directly."""
        pr = Mock(spec=github.PullRequest.PullRequest)
        pr.number = 1
        pr.state = "open"
        pr.draft = False
    ...
    def test_eligible_pr_with_no_reviewers_gets_needs_triage(self):
        """PR eligible for triage and no reviewers should get needs-triage."""
        pr = Mock(spec=github.PullRequest.PullRequest)
        pr.number = 1
        pr.state = "open"
        pr.draft = False
        ...
        self.labeler._apply_label.assert_called_once_with(pr, "status:needs-triage")
